...
filed yet → stop, tell the user to file the issue first. Don't create it for them.
...
6. **[SUBAGENT: fresh context] → [SUBAGENT: coder]**, per vertical slice — one seam, one red
   test written with zero context from step 3's reasoning, one minimal implementation. tdd-guard
   enforces red-before-green on every slice. Run `explain-it slice <n> <slug>` after each slice.
