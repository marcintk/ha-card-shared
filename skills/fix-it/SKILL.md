...
2. **[SUBAGENT: Explore]** Reproduce, read-only — locate the code path, gather concrete evidence
   (command output, an instrumented value), name what's ruled out.
...
9. On accept: **[SKILL: explain-it]** run `explain-it finalize <n> <slug>` — it sets the note to
   ...
   snapshot.
10. **[HUMAN]** Wait for "merge it". Confirm the explain-diff HTML was reviewed, then `gh run watch`
