...
2. Run `/ponytail-audit` (full repo scan). Any finding → fix on a branch, PR, merge, then
   re-run. Only continue once the audit comes back clean.
3. **[HUMAN]** Run `/improve-codebase-architecture` (it is `disable-model-invocation: true` — the
   human runs it, not the skill). Any deepening opportunity you accept → its own branch, PR,
   merge, then re-run from step 1. Only continue once nothing is left worth taking.
4. **[HUMAN]** Determine the semver bump — state it, don't just pick it silently:
...
5. Bump version: `npm version patch|minor|major` (updates `package.json`, commits, creates the
   local tag).
6. Push: `git push --follow-tags`. CI creates the GitHub Release as a **draft** — every release,
   always.
7. Write the customer-facing release notes into the draft: `gh release edit <tag> --notes-file -`
...
8. **[HUMAN]** Publish the draft from the GitHub Releases UI once the notes and the
